...
fn split_line_in_half(input: &String) -> (&str, &str) {
    ...
    (
        input.substring(0, line_len / 2),
        input.substring(line_len / 2, line_len),
    )
}
...
fn find_wrong_rucksack_item(rucksack: &RuckSack) -> u32 {
    let compartment1_matches: Vec<&u32> = rucksack
        .compartment_1
        .iter()
...
fn find_shared_priority(input: &Vec<RuckSack>) -> u32 {
    ...
    for (i, ruck_sack) in input.iter().enumerate() {
        ...
        remaining_prios.clone().iter().for_each(|c_type| {
            if !ruck_sack.compartment_1.contains(c_type)
                && !ruck_sack.compartment_2.contains(c_type)
            {
