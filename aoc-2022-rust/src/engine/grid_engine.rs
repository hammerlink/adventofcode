use std::{collections::HashMap, borrow::{Borrow, BorrowMut}, rc::Rc, cell::{RefCell, Ref, RefMut}};
...
impl<T: Clone> Grid<T> {
    pub fn set_cell_value(&mut self, x: isize, y: isize, value: T) -> Rc<RefCell<GridCell<T>>> {
        ...
        x_map.insert(x, Rc::new(RefCell::new(GridCell {
            x,
            y,
            value,
        })));
    ...
    pub fn get_cell(&self, x: isize, y: isize) -> Option<Rc<RefCell<GridCell<T>>>> {
        if !self.map.contains_key(&y) { return None; }
        let y_map = self.map.get(&y).unwrap();
        if !y_map.contains_key(&x) { return None; }
    ...
    pub fn new_from_lines(lines: Vec<String>, parse_line: fn(String) -> Vec<T>, min_x_arg: Option<isize>, min_y_arg: Option<isize>) -> Self {
        ...
        for (y, line) in lines.iter().enumerate() {
            ...
            if !map.contains_key(&(y as isize)){ map.insert(y as isize, HashMap::new()); }
            let parsed_values = parse_line(line.clone()); 
            for (x, value) in parsed_values.into_iter().enumerate() {
    ...
    pub fn print(&self, print_value: fn(Option<T>) -> String) {
        println!("x: {} - {} y: {} - {}", self.min_x, self.max_x, self.min_y, self.max_y);
