...
import IPassPort = Y2020_Day4.IPassPort;

export namespace Y2020_Day4 {

    export const requiredFields = ['byr','iyr','eyr','hgt','hcl','ecl','pid',];
    ...
    export function isValidPassPort(passPort: IPassPort): boolean {
        for (let i = 0; i < requiredFields.length; i++) if (passPort[requiredFields[i]] === undefined) return false;
        return true;
    }
    ...
    export function getPassPorts(lines: string[]): IPassPort[] {
        ...
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i];
            if (line.length === 0) {
                if (currentSection.length) passPorts.push(convertToPassport(currentSection));
                currentSection  = [];
...
if (!module.parent) {
    ...
    async function main() {
        const lines = await FileEngine.readFileToLines(path.join(path.dirname(__filename), './data/y2020_day4.input'), false);
