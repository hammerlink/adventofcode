...
export namespace Y2020_Day7 {
  ...
  export function parseBag(line: string) {
    const bag: Bag = createRawBag(
      line.match(/^([a-z]+\s[a-z]+) bags contain/)[1],
    );

    const rawBagRules = line.match(/(\d+ [a-z]+ [a-z]+ bags?)/g);
    if (!rawBagRules || rawBagRules.length === 0) return;
    const bagRules: { amount: number; color: string }[] = (
      rawBagRules as string[]
    )
      .map((bagRule: string) => bagRule.match(/(\d+) ([a-z]+\s[a-z]+)/))
      .map<{ amount: number; color: string }>((result) => ({
  ...
  export function countParents(
    color: string,
    colors: { [color: string]: { path: string[] } },
    path: string[],
  ...
  export function countChildren(
    color: string,
    colors: { [color: string]: { childrenCount: number } },
  ): number {
    const bag = bagTree[color];
    if (colors[color]) {
...
if (!module.parent) {
  ...
  async function main() {
    const exampleLines = await FileEngine.readFileToLines(
      path.join(path.dirname(__filename), "./data/y2020_day7.example"),
    );
    assert.equal(
      Y2020_Day7.part1(exampleLines, "shiny gold"),
      4,
      "example 1 part 1",
    );
    assert.equal(
      Y2020_Day7.part2(exampleLines, "shiny gold"),
      32,
      "example 1 part 1",
    );
...
dark violet bags contain no other bags.`;
    expect(
      Y2020_Day7.part2(example2.split("\n"), "shiny gold"),
      "part 2, example 2 failing",
    ).equals(126);

    const lines = await FileEngine.readFileToLines(
      path.join(path.dirname(__filename), "./data/y2020_day7.input"),
    );
