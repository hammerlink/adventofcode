...
export namespace Y2020_Day20 {
  ...
  export function getAllBorders(map: BasicMap<TilePoint>): {
    [orientationKey: number]: string;
  } {
    ...
    for (let i = minX; i <= maxX; i++) {
      borders[Orientation.NORTH] += MapEngine.getPoint(
        map,
        i,
        minX,
      ).value.value;
      borders[Orientation.SOUTH] += MapEngine.getPoint(
        map,
        i,
        maxX,
      ).value.value;
  ...
  export function mergeTiles(
    ...
    const transformation = transformTile(
      borderMatchA.orientation as Orientation,
      borderMatchB.orientation as Orientation,
      borderMatchA.reverse !== borderMatchB.reverse,
      tileB.map,
    );
    if (transformation.altered) {
      ...
      if (
        newBorders[
          oppositeOrientation(borderMatchA.orientation as Orientation)
        ] !== tileA.borders[borderMatchA.orientation]
      ) {
        const x = null;
      }
    ...
    const { x, y } = getDirection(
      borderMatchA.orientation as Orientation,
      tileA.imagePosition.x,
      tileA.imagePosition.y,
    );
    const existingPoint = MapEngine.getPoint(imageMap, x, y);
    if (existingPoint) {
      MapEngine.printMap(imageMap, (x) =>
        !!x?.value.tile.tileId ? "" + x?.value.tile.tileId : "----",
      );
  ...
  ): { orientation: string; reverse: boolean }[] {
    const matches = [];
    for (const orientation in tile.borders) {
      if (tile.borders[orientation] === border)
        matches.push({ orientation, reverse: false });
      else if (tile.borders[orientation] === reverseBorder)
        matches.push({ orientation, reverse: true });
    }
    if (matches.length > 1)
      throw new Error("tile with multiple identitical sides");
    return matches;
  }
  ...
  export function transformTile(
    ...
    reverse: boolean,
    tileMap: BasicMap<TilePoint>,
  ): { map: BasicMap<TilePoint>; altered: boolean; defaultReverse: boolean } {
    ...
    const defaultReverse =
      orientationAction[startOrientation].defaultReverse[tileOrientation];
  ...
  export function mapAllBorderMatches(
    tiles: Tile[],
  ): { border: string; tiles: Tile[]; done?: boolean }[] {
    ...
    tiles.forEach((tile) => {
      for (const orientation in tile.borders) {
        ...
        else if (borderMatches[reverseBorder])
          borderMatches[reverseBorder].push(tile);
  ...
  export function part1(lines: string[]): number {
    ...
    const leftBottom = MapEngine.getPoint(
      imageMap,
      imageMap.minX,
      imageMap.maxY,
    ).value.tile.tileId;
    const leftTop = MapEngine.getPoint(imageMap, imageMap.minX, imageMap.minY)
      .value.tile.tileId;
    const rightBottom = MapEngine.getPoint(
      imageMap,
      imageMap.maxX,
      imageMap.maxY,
    ).value.tile.tileId;
    const rightTop = MapEngine.getPoint(imageMap, imageMap.maxX, imageMap.minY)
      .value.tile.tileId;
    return leftBottom * leftTop * rightBottom * rightTop;
  }

  export function writeTileContent(
    tile: Tile,
    xMap: number,
    yMap: number,
    tileMap: BasicMap<TilePoint>,
  ...
  export function markSeaMonster(
    tileMap: BasicMap<TilePoint>,
    x: number,
    y: number,
  ): boolean {
  ...
  export function isStartOfSeaMonster(
    tileMap: BasicMap<TilePoint>,
    x: number,
    y: number,
  ): boolean {
  ...
  export function part2(lines: string[]): number {
    ...
    for (let y = imageMap.minY; y <= imageMap.maxY; y++) {
      for (let x = imageMap.minX; x <= imageMap.maxX; x++) {
        writeTileContent(
          MapEngine.getPoint(imageMap, x, y).value.tile,
          product(x, 8),
          product(y, 8),
          tileMap,
        );
...
if (!module.parent) {
  ...
  async function main() {
    const exampleLines = await FileEngine.readFileToLines(
      path.join(path.dirname(__filename), "./data/y2020_day20.example"),
      false,
    );

    assert.strictEqual(
      Y2020_Day20.part1(exampleLines),
      20899048083289,
      "example 1 part 1",
    );

    assert.strictEqual(
      Y2020_Day20.part2(exampleLines),
      273,
      "example 1 part 2",
    );

    const lines = await FileEngine.readFileToLines(
      path.join(path.dirname(__filename), "./data/y2020_day20.input"),
      false,
    );
  ...
  (async () => {
    ...
    console.log("total", totalMs, "average ms", (totalMs / 100).toFixed(2));
